...
# Hinge loss with original features.
def TrainSVM(train_features, train_labels, valid_features, valid_labels,
    ...
    x2 = tf.sub(tf.matmul(features, W), b)
    x3 = tf.mul(label, x2)

    x4 = 1 - x3
    x5 = tf.nn.relu(x4)
    loss = tf.reduce_sum(x5)
    optimizer = tf.train.GradientDescentOptimizer(0.001).minimize(loss)
    prediction = tf.argmax(x2, 1)
    ...
    for epoch_i in range(num_epochs):
        for batch_i in range(num_instances // batch_size):
            ...
            batch_features = train_features[start_index: stop_index, :]
...
def TrainKSVM(train_features, train_labels, valid_features, valid_labels,
    ...
    for i in range(num_instances):
        ...
        ktrain_features[i, :] = np.exp(-lamda * diff)
    ...
    for i in range(num_valid_instances):
        ...
        diff = np.sum(diff * diff, axis=1)
        diff.shape = (1, num_kfeatures)
        kvalid_features[i, :] = np.exp(-lamda * diff)
